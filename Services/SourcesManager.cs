...
public sealed class SourcesList
{
    public List<string> Subscriptions { get; } = [];
    public List<string> CustomServers { get; } = [];
}

public static class SourcesManager
{

    public static SourcesList Load(string filePath)
    {
        var result = new SourcesList();
        ...
        foreach (var line in File.ReadAllLines(filePath))
        {
            ...
            if (trimmed.StartsWith("http://", StringComparison.OrdinalIgnoreCase) ||
                ...
                result.Subscriptions.Add(trimmed);
            else if (IsVpnUri(trimmed))
                result.CustomServers.Add(trimmed);
        }

        return result;
    }

    public static void Save(string filePath, SourcesList sources)
    {
        var lines = new List<string>
        {
            "# VPNCheck — источники конфигураций",
            "# ════════════════════════════════════════════════════════════════════════════",
            "# URL (http:// или https://)              → ссылка на подписку",
            "# VPN конфиг (vless://, trojan://, ...)   → собственный сервер",
            "# Строки с # и пустые строки игнорируются",
            "# Управление источниками: запустите VPNCheck → «Источники и подписки»",
            "# ════════════════════════════════════════════════════════════════════════════",
            ""
        };

        if (sources.Subscriptions.Count > 0)
        {
            lines.Add("# ── Подписки ──────────────────────────────────────────────────────────────");
            lines.AddRange(sources.Subscriptions);
            lines.Add("");
        }

        if (sources.CustomServers.Count > 0)
        {
            lines.Add("# ── Собственные серверы ───────────────────────────────────────────────────");
            lines.AddRange(sources.CustomServers);
        }
    ...
    public static void EnsureDefaultFile(string filePath)
    {
        ...
        var defaults = new SourcesList();
        foreach (var url in DefaultSubscriptions)
            defaults.Subscriptions.Add(url);
        Save(filePath, defaults);
    }

    public static bool IsVpnUri(string s) => VpnProtocols.IsVpnUrl(s);
